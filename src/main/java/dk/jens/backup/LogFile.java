...
public class LogFile implements Parcelable {
    ...
    private String packageLabel, packageName, versionName, sourceDir, cpuArch, backupDate;
    private int[] iv;
    ...
    public LogFile(File backupSubDir, String fileName) {
        FileReaderWriter frw = new FileReaderWriter(backupSubDir.getAbsolutePath(), fileName + ".properties");
        ...
        try {
            ...
            arrJson = jsonObject.optJSONArray("iv");
            int[] arrInt = new int[Objects.requireNonNull(arrJson).length()];
            for (int i = 0; i < arrJson.length(); i++)
                arrInt[i] = arrJson.getInt(i);
            this.iv = arrInt;
            ...
            this.packageLabel = this.packageName = this.versionName = this.sourceDir = this.cpuArch = this.backupDate = "";
            this.iv = new int[0];
            this.versionCode = 0;
    ...
    public static void writeLogFile(File backupSubDir, AppInfo appInfo, int backupMode, boolean encrypted) {
        try {
            ...
            jsonObject.put("splitSourceDirs", appInfo.getSplitSourceDirs() == null ?
                    new JSONArray() : new JSONArray(appInfo.getSplitSourceDirs()));
            jsonObject.put("cpuArch", Build.SUPPORTED_ABIS[0]);
            byte[] iv = Utils.initIv();
            jsonObject.put("iv", new JSONArray(iv));
            ...
            File outFile = new File(backupSubDir.getParent(), backupSubDir.getName() + ".properties");
    ...
    public void writeToParcel(Parcel out, int flags) {
        ...
        out.writeString(cpuArch);
        out.writeIntArray(iv);
    ...
    private LogFile(Parcel in) {
        ...
        cpuArch = in.readString();
        iv = in.createIntArray();
