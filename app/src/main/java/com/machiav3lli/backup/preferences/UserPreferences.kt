    ...
    AppTheme(
        ...
        LazyColumn(
            ...
            items(items = prefs) { pref ->
                when (pref) {
                    DeviceLockPref -> SwitchPreference(pref = pref as BooleanPref,
                        isEnabled = context.isDeviceLockAvailable()
                    ) {
                        isDeviceLockEnabled = it
                    }
                    BiometricLockPref -> SwitchPreference(pref = pref as BooleanPref,
                        isEnabled = context.isBiometricLockAvailable() && isDeviceLockEnabled
                    )
                    is ListPref -> ListPreference(pref = pref) {
                        dialogsPref = pref
                        openDialog.value = true
                    }
                    is EnumPref -> EnumPreference(pref = pref) {
                        dialogsPref = pref
                        openDialog.value = true
                    }
                    BackupFolderPref -> LaunchPreference(pref = pref,
                        summary = backupDir
                    ) {
                        launcher.launch(BACKUP_DIRECTORY_INTENT)
                    }
                    is BooleanPref -> SwitchPreference(pref = pref)
                    is IntPref     -> SeekBarPreference(pref = pref)
                }
            }
        }
